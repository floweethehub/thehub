...
static CBlock CreateGenesisBlock(const char* pszTimestamp, const CScript& genesisOutputScript, uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, int64_t genesisReward)
{
 ...
 *   vMerkleTree: 4a5e1e
 */
static CBlock CreateGenesisBlock(uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, int64_t genesisReward)
{
...
public:
    CScaleNetParams() {
        ...
        checkpointData = CCheckpointData{
            ...
            (9999, uint256S("0x000000007fb3362740efd1435aa414f54171993483799782f83c61bc7bf1b1be"))
            ,
            1601685801, // * UNIX timestamp of last checkpoint block
            12467,      // * total number of transactions between genesis and last checkpoint
                        //   (the tx=... number in the SetBestChain hub.log lines)
            90000       // * estimated number of transactions per day after checkpoint
        };
    }
};
