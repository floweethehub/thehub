...
static bool DecryptKey(const CKeyingMaterial& vMasterKey, const std::vector<unsigned char>& vchCryptedSecret, const CPubKey& vchPubKey, CKey& key)
{
    ...
    if(!DecryptSecret(vMasterKey, vchCryptedSecret, vchPubKey.createHash(), vchSecret))
        return false;
...
{
    {
        ...
        if (!EncryptSecret(vMasterKey, vchSecret, pubkey.createHash(), vchCryptedSecret))
            return false;
...
{
    {
        ...
        BOOST_FOREACH(KeyMap::value_type& mKey, mapKeys)
        {
            ...
            CKeyingMaterial vchSecret(key.begin(), key.end());
            std::vector<unsigned char> vchCryptedSecret;
            if (!EncryptSecret(vMasterKeyIn, vchSecret, vchPubKey.createHash(), vchCryptedSecret))
                return false;
