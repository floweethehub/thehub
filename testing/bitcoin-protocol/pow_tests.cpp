...
static bool wasAsertEnabled(int64_t activationTime, const CBlockIndex *pindexPrev) {
    assert(pindexPrev);
    return pindexPrev->GetMedianTimePast() >= activationTime;
}
 ...
 * of an anchor block.
 */
void POWTests::asert_activation_anchor_test()
{
    ...
    params.hf201711Height = 2016;
    const int64_t activationTime = params.hf202011Time;
    CBlockHeader blkHeaderDummy;
    ...
    for (int i = 1; i < 2000 + int(insecure_rand() % 1000); i++) {
        blocks[bidx] = GetBlockIndex(&blocks[bidx-1], 600, initialBits);
        bidx++;
        QVERIFY(bidx < int(blocks.size()));
    }
    ...
    QCOMPARE(pindexPreActivation->nTime, activationTime + 5*500 - 1);
    QCOMPARE(pindexPreActivation->GetMedianTimePast(), activationTime - 1);
    // QVERIFY(IsDAAEnabled(params, pindexPreActivation));
    assert(pindexPreActivation->nHeight >= params.hf201711Height);
    ...
    QVERIFY(!wasAsertEnabled(activationTime, pindexPreActivation));
    QCOMPARE(CalculateNextWorkRequired(pindexPreActivation, &blkHeaderDummy, params), 0x180236e1);
    ...
    QVERIFY(wasAsertEnabled(activationTime, &indexActivation0));
    QCOMPARE(CalculateNextWorkRequired(&indexActivation0, &blkHeaderDummy, params), 0x180236e1);
    ...
    QVERIFY(wasAsertEnabled(activationTime, &indexActivation1));
    // cache will be stale here, and we should get the right result regardless:
    QCOMPARE(CalculateNextWorkRequired(&indexActivation1, &blkHeaderDummy, params), 0x180232fd);
    // second call will have used anchor cache, shouldn't change anything
    QCOMPARE(CalculateNextWorkRequired(&indexActivation1, &blkHeaderDummy, params), 0x180232fd);
    ...
    QVERIFY(wasAsertEnabled(activationTime, &indexActivation2));
    QCOMPARE(CalculateNextWorkRequired(&indexActivation2, &blkHeaderDummy, params), anchorBits2);
    ...
    QVERIFY(wasAsertEnabled(activationTime, &indexActivation2));
    QCOMPARE(CalculateNextWorkRequired(&indexActivation3, &blkHeaderDummy, params), 0x1d00ffff);
    ...
    QCOMPARE(CalculateNextWorkRequired(&indexActivation3_return, &blkHeaderDummy, params), anchorBits3);
    // Retry for cache
    QCOMPARE(CalculateNextWorkRequired(&indexActivation3_return, &blkHeaderDummy, params), anchorBits3);
    ...
    QCOMPARE(indexActivation4.GetMedianTimePast(), activationTime);
    QVERIFY(wasAsertEnabled(activationTime, &indexActivation4));
    QCOMPARE(CalculateNextWorkRequired(&indexActivation4, &blkHeaderDummy, params), 0x18010db3);
    ...
    for (int i = 1; i < 1000; i++) {
        QVERIFY(bidx < int(blocks.size()));
        ResetASERTAnchorBlockCache();
        uint32_t nextBits = CalculateNextWorkRequired(pindexChain2, &blkHeaderDummy, params);
        blocks[bidx] = GetBlockIndex(pindexChain2, insecure_rand() % 1200, nextBits);
    ...
    for (CBlockIndex *pindex = pindexChain2; pindex != &indexActivation2 ; pindex = pindex->pprev) {
        uint32_t nextBits = CalculateNextWorkRequired(pindex->pprev, &blkHeaderDummy, params);
        QCOMPARE(nextBits, pindex->nBits);
    }
}
